...
public interface SpatialAccelerationReadOnly extends SpatialMotionReadOnly
{
    ...
    * </p>
    * <p>
    * Effectively, the resulting linear acceleration &alpha;<sub>result</sub> is calculated as
    * follows:
    *
    * <pre>
    ...
    * </pre>
    *
    * where &alpha;<sub>this</sub> and &omega;'<sub>this</sub> represent the angular and linear
    * parts of this spatial acceleration, &omega;<sub>twist</sub> and &nu;<sub>twist</sub> represent
    * the angular and linear parts of the body twist, and {@code P} is the {@code bodyFixedPoint}.
    * </p>
    *
    * @param bodyTwist the twist of {@code this.bodyFrame} with respect to {@code this.baseFrame}
    *           and expressed in {@code this.expressedInFrame}. Not modified.
    ...
    * @throws ReferenceFrameMismatchException if the {@code bodyTwist} does not have the same
    *            reference frames as {@code this}, if the {@code bodyFixedPoint} is not expressed in
    *            the same reference frame as {@code this}, or if this spatial acceleration is not
   ...
   {
      /*
       * Without this check, the "expressed-in-frame" could possibly be moving with respect to both
       * the base and the body. In such context, this acceleration would consider additional
       * acceleration induced by the motion of the "expressed-in-frame" itself which is undesirable
       * here.
       */
      ...
      /*
       * A spatial acceleration can be changed from being expressed in the base frame to being
       * expressed in the body frame and vice versa without considering the twist between the two.
       * Indeed when looking at the equations provided in
       * SpatialAccelerationVectorBasics.changeFrame(ReferenceFrame, Twist, Twist), the bias
       * acceleration generated by the velocity becomes zero in such condition. Knowing that, there
       * is no need to enforce a particular frame between the base frame and the body frame, the
       * resulting acceleration will be correct, just with components expressed in a different
       * coordinate system.
       */
    ...
    * {@link #getLinearAccelerationAt(TwistReadOnly, FramePoint3DReadOnly, FrameVector3DBasics)}
    * with the {@code bodyfixedPoint} being zero. So effectively, the linear acceleration
