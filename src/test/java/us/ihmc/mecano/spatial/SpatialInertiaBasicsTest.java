...
public abstract class SpatialInertiaBasicsTest<T extends SpatialInertiaBasics>
{
   private static final double EPSILON = 1.0e-12;
   private static final int ITERATIONS = 1000;

   public abstract T copySpatialInertia(SpatialInertiaReadOnly other);
   ...
   public void testApplyInverseTransform() throws Exception
   {
      ...
      { // Testing applyInverseTransform against applyTransform
         ...
         T actual = copySpatialInertia(expected);
      ...
      { // Test different transform types
         ...
         actual = copySpatialInertia(expected);
         expected.applyInverseTransform(transform);
   ...
   public void testComputeKineticCoEnergy() throws Exception
   {
      ...
      { // Test that the class is using the method from RigidBodyDynamicsTools.
         ...
         T inertia = copySpatialInertia(spatialInertia);
         Twist twist = MecanoRandomTools.nextTwist(random, bodyFrame, worldFrame, expressedInFrame);

         double expected = MecanoTools.computeKineticCoEnergy(spatialInertia.getMomentOfInertia(),
                                                              spatialInertia.getMass(),
                                                              spatialInertia.getCenterOfMassOffset(),
   ...
   public void testComputeDynamicWrenchFast() throws Exception
   {
      ...
      { // Test that the class is using the method from RigidBodyDynamicsTools.
         ReferenceFrame bodyFrame = EuclidFrameRandomTools.nextReferenceFrame(random);
         SpatialInertia spatialInertia = MecanoRandomTools.nextSpatialInertia(random, bodyFrame, bodyFrame);
         spatialInertia.getCenterOfMassOffset().setToZero();
         T inertia = copySpatialInertia(spatialInertia);
         ...
         MecanoTools.computeDynamicMomentFast(spatialInertia.getMomentOfInertia(), acceleration.getAngularPart(), twist.getAngularPart(), expected.getAngularPart());
         MecanoTools.computeDynamicForceFast(spatialInertia.getMass(),
   ...
   public void testChangeFrame() throws Exception
   {
      ...
      for (int i = 0; i < ITERATIONS; i++)
      {
         ...
         SpatialInertia spatialInertia = MecanoRandomTools.nextSpatialInertia(random, bodyFrame, initialFrame);
         T inertia = copySpatialInertia(spatialInertia);
         Twist twist = MecanoRandomTools.nextTwist(random, bodyFrame, baseFrame, initialFrame);

         assertSame(inertia.getReferenceFrame(), initialFrame);
         ...
         assertSame(inertia.getReferenceFrame(), finalFrame);
      ...
      { // Test against the matrix equation using the adjoint.
         ...
         SpatialInertia spatialInertia = MecanoRandomTools.nextSpatialInertia(random, initialFrame, initialFrame);
         T inertia = copySpatialInertia(spatialInertia);
   ...
   public void testTransform()
   {
      ...
      { // Test against EJML
         ...
         inertia.get(spatialInertia_ejml);
         ...
         inertia.transform(vectorOriginal, actualVectorTransformed);
    ...
    */
   private double computeKineticCoEnergy(Twist twist, T inertia)
   {
   ...
   private Wrench computeWrench(SpatialAcceleration acceleration, T inertia)
   {
